...
```{r}
corr_mod <- corr_sum %>%
  do(tidy(standardize(lm_robust(formula = opct ~ white + poverty + educ + foreign_born, data = .))))
```

```{r}
between_counties_plot <- corr_mod %>%
  filter(!str_detect(term, "(Intercept)")) %>%
  ggplot(aes(x = urban_suburban_rural)) +
  geom_col(aes(y = estimate, fill = term), position = "dodge") +
  geom_errorbar(aes(ymax = conf.high, ymin = conf.low, fill = term), position = "dodge") +  
...
within_counties_plot <- dd_lm_mean %>%
  ...
  geom_jitter(aes(y = estimate), alpha = 0.2, shape = 21) + 
  geom_hline(yintercept = 0, linetype = "dashed", col = "red") +
  coord_flip() +
  labs(title = "Within counties (urban areas)",
...
ggsave(here("outputs", "county_desc_unit_diff.png"), height = 8, width = 15)
```