...
```{r}
mbf <- read_csv(here("raw_data", "irs_mbf.csv"))

org_activites <- read_csv(here("raw_data", "irs_org_activities.csv"))
...
civic opps = if an org does any of the following: offering volunteer opportunities / membership / ways to take civic or political action / community events

```{r}
org_activites_mbf <- org_activites %>% left_join(mbf %>% dplyr::select(ein, state, city, zip))
...
org_sums <- org_activites_mbf %>%
  group_by(ZCTA) %>%
  summarize(
    vol_sum = sum(volunteer, na.rm = T),
    mem_sum = sum(membership, na.rm = T),
    action_sum = sum(take_action, na.rm = T),
    event_sum = sum(events, na.rm = T),
    opp_binary_sum = sum(opp_binary, na.rm = T),
    opp_mean_sum = sum(opp_mean, na.rm = T),
    opp_invcov_sum = sum(opp_invcov, na.rm = T)
  )

png(
  file = here("outputs", "civic_opportunity_index.png"),
  width = 3.25,
  height = 3.25,
  units = "in",
  res = 600,
...
cor(org_sums %>% dplyr::select(matches("_sum"))) %>%
  corrplot(
    tl.col = "black",
    method = "number"
  )
