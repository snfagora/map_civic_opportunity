...
org_activites_mbf <- org_activites_mbf %>%
  ...
  mutate(opp_mean = idx_mean(volunteer, membership, take_action, events),
         opp_invcov = idx_invcov(volunteer, membership, take_action, events))

org_sums <- org_activites_mbf %>%
  group_by(ZCTA) %>%
  summarize(vol_sum = sum(volunteer, na.rm = T),
...
png(file = here("outputs", "civic_opportunity_index.png"),     
  width     = 3.25,
  height    = 3.25,
  units     = "in",
  res = 600,
