...
```{r}
org_activities_combined <- org_activities %>% 
  left_join(irs_activities) %>% 
  mutate(volunteer = if_else(volunteer_text==1,1,volunteer, membership = if_else(member_text==1,1,membership)))

org_activites_mbf <- org_activites_combined %>% 
...
# Jae, see addition of irs_activities above
...
cnts_counts <- org_activities_mbf %>% 
  ...
  summarise(m=max(idx_var, na.rm=T)) %>% 
  group_by(FIPS) %>% 
  summarise(n=n(), mean_sum=sum(m))
