...
```{r}
org_activities_mbf %>% group_by(is_po) %>% count(website)
...
dd %>%
  ...
  summarize(mean_opc = mean(opc_tile, na.rm = T),
            sd = sd(opc_tile, na.rm = T))
```

```{r}
dd %>%
  group_by(State, urban_suburban_rural) %>% 
  summarize(mean_opc = mean(opc_tile, na.rm = T),
            sd = sd(opc_tile, na.rm = T)) %>%
  filter(urban_suburban_rural == "Urban") %>%
  arrange(desc(mean_opc))
```
...
dd %>%
  ...
  summarize(mean_opc = mean(opc_tile, na.rm = T),
            sd = sd(opc_tile, na.rm = T)) %>%
  filter(urban_suburban_rural == "Rural") %>%
  arrange(desc(mean_opc))
```
...
ggsave(here("outputs", "pred_plot.png"))
```
