...
# if the following pkg is not installed
...
(org_activities_mbf %>%
  ...
  nrow()) / nrow(org_activities_combined)
```
...
poverty2_plot <- dd %>%
  ggplot(aes(x = opc, y = per_poverty)) +
  geom_jitter(aes(x = opc), alpha = 0.05, color = "blue") +
  geom_smooth(aes(x = opc), method = "lm", level = 0.95, color = "blue") +
  geom_jitter(aes(x = all_org), alpha = 0.05, color = "red") +
  geom_smooth(aes(x = all_org), method = "lm", level = 0.95, color = "red") +
  labs(
    x = glue("Civic opportunity scores
                per capita"),
    y = "Federal poverty level"
...
edu2_plot <- dd %>%
  ggplot(aes(y = college_educ)) +
  geom_jitter(aes(x = opc), alpha = 0.05, color = "blue") +
  geom_smooth(aes(x = opc), method = "lm", level = 0.95, color = "blue") +
  geom_jitter(aes(x = all_org), alpha = 0.05, color = "red") +
  geom_smooth(aes(x = all_org), method = "lm", level = 0.95, color = "red") +
  labs(
    x = glue("Per capita measures"),
    y = "College educated"
  ) +
  scale_y_continuous(label = scales::percent, limits = c(0, 1)) +
  xlim(c(0, 5))
...
poverty2_plot + edu2_plot + white2_plot + plot_annotation(tag_levels = "A")

ggsave(here("outputs", "civic_inequ_asso2.png"), width = 9, height = 7)
```

```{r}
mod_comp_list <- list(
  "Civic opportunity" = standardise(lm_robust(opc ~ per_poverty, data = dd)),
  "Organizational density" = standardise(lm_robust(all_org ~ per_poverty, data = dd)),
  "Civic opportunity" = standardise(lm_robust(opc ~ college_educ, data = dd)),
  "Organizational density" = standardise(lm_robust(all_org ~ college_educ, data = dd)),
  "Civic opportunity" = standardise(lm_robust(opc ~ race_per_white_nonhispanic, data = dd)),
  "Organizational density" = standardise(lm_robust(all_org ~ race_per_white_nonhispanic, data = dd))
)
```
