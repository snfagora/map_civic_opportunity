...
poverty_plot <- dd %>%
  ...
  ylim(c(0, 1)) +
  labs(x = glue("Civic opportunity scores 
                per capita"),
...
```{r}
bind_rows(lm_robust(opc ~ per_poverty, data = dd) %>% tidy() %>% slice(-1),
lm_robust(opc ~ college_educ, data = dd) %>% tidy() %>% slice(-1),
lm_robust(opc ~ race_per_white_nonhispanic, data = dd) %>% tidy() %>% slice(-1)) %>%
   mutate(term = recode(term,
    "race_per_white_nonhispanic" = "Non-hispanic white (%)",
    "per_poverty" = "Federal poverty level (%)",
    "college_educ" = "College educated (%)"
...
zcta_combined <- zcta_cnts %>% right_join(
    zcta_detail %>%
