...
```{r}
org_activites_mbf$website %>% sum() # 894,335
...
cnts_counts <- org_activities_mbf %>% 
  ...
  summarise(m = max(opp_mean, na.rm = T)) %>% # Milan: this should be the average of the binary dimenisons. 
  group_by(FIPS) %>% 
