...
```{r}
# this is the IRS master file
mbf <- vroom(here("raw_data", "irs_mbf.csv"))

# organization activities from the websites and elsewhere 
org_activities <- vroom(here("raw_data", "irs_org_activities.csv"))

# organization activities from the IRS tax returns
irs_activities <- vroom(here("raw_data","irs_nonweb_activities.csv"))
  replace_na(list(volunteer_text = 0, member_text = 0))
...
# County-level social capital data 

## Kyne and Aldrich index 
...
# ZCTA-level civic opportunity data for the LA map

zcta_cnts <- read_csv(here("raw_data", "zcta_counts_idx_mean_nonpo.csv"))

# ZCTA-level demographic data

zcta_detail <- read_csv(here("raw_data", "census_zcta_detail_data.csv"), col_types = (GEOID <- "c"))

# Shape files for the LA map
...
# 1,774,798, approximately 1.8M organizations
...
org_activities_mbf$extractable %>% sum() / nrow(org_activities_mbf) # 50%
...
(org_activities_mbf %>%
  ...
  nrow()) / nrow(org_activities_combined) # 30%
```
...
* The following three measures are scored on a 0-1 scale.

* The objective of this exercise is to test construct validity.
...
org_activities_mbf <- org_activities_mbf %>%
  rename(zcta = ZCTA) %>%
  select(ein, state, city, zip, fips, zcta, is_po, take_action, volunteer, membership, events, opp_binary, opp_mean, opp_invcov, opp_princomp)

write_csv(org_activities_mbf, here("processed_data", "org_measures.csv"))

dim(org_activities_mbf)
...
dd <- dd %>% left_join(penn_sc %>%
  mutate(FIPS = as.character(FIPS)) %>%
  select(FIPS, sk2014))

# join with the 
dd <- dd %>% left_join(chetty_sc)

# Kyne and Aldrich 
cor.test(dd$opc, dd$socialcap)$estimate %>% round(2)

# Rupasingha 
cor.test(dd$opc, dd$sk2014)$estimate %>% round(2)

# Chetty et al 
cor.test(dd$opc, dd$civic_organizations_county)$estimate %>% round(2)
```
...
poverty_plot <- dd %>%
  ggplot(aes(x = opc, y = per_poverty)) +
  geom_jitter(alpha = 0.05) +
  geom_smooth(method = "lm", level = 0.95) +
  labs(
    x = glue("Civic opportunity scores"),
...
college_edu_plot <- dd %>%
  ggplot(aes(x = opc, y = college_educ)) +
  geom_jitter(alpha = 0.05) +
  geom_smooth(method = "lm", level = 0.95) +
  labs(
    x = glue("Civic opportunity scores"),
...
white_plot <- dd %>%
  ggplot(aes(x = opc, y = race_per_white_nonhispanic)) +
  geom_jitter(alpha = 0.05) +
  geom_smooth(method = "lm", level = 0.95) +
  labs(
    x = glue("Civic opportunity scores"),
...
poverty_plot + college_edu_plot + white_plot + plot_annotation(tag_levels = "A")

ggsave(here("outputs", "civic_inequ_asso.png"), width = 9, height = 7)
```
...
```{r}
# ggplot version with the state border
...
la_map <- ggplot() +
  ...
  scale_fill_manual(labels = c(1:5, "No Data"), values = my_palette) +
  geom_sf(data = la_zip_shp_no_islands, aes(geometry = geometry), fill = "transparent", color = gray(0.8), inherit.aes = FALSE) +
  theme_bw() +
  theme(legend.position = "bottom", axis.line = element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank(), axis.ticks = element_blank(), axis.title.x = element_blank(), axis.title.y = element_blank()) +
  labs(fill = "Civic opportuntiy index")
```
