...
write_csv(zcta_counts, here("outputs", "zcta_counts.csv"))

city_counts <- org_activities_mbf %>%
  group_by(state, city, grouping_value) %>%
  summarise(m = max(opp_mean, na.rm = T)) %>%
  group_by(state, city) %>%
  summarise(
    n = n(),
...
write_csv(city_counts, here("outputs", "city_counts.csv"))
```
...
dd_fb$sk2014 <- range01(dd_fb$sk2014)

dd_fb <- dd_fb %>%
  mutate(diff = abs(opc - sk2014))
```

```{r}
mod_comp_list <- list(
  "Civic opportunity" = standardise(lm_robust(opc ~ per_poverty, data = dd_fb)),
  "Rupasingha et al Index" = standardise(lm_robust(sk2014 ~ per_poverty, data = dd_fb)),
  "Absolute difference between two measures" = standardise(lm_robust(diff ~ per_poverty, data = dd_fb)),
  "Civic opportunity" = standardise(lm_robust(opc ~ college_educ, data = dd_fb)),
  "Rupasingha et al Index" = standardise(lm_robust(sk2014 ~ college_educ, data = dd_fb)),
  "Absolute difference between two measures" = standardise(lm_robust(diff ~ college_educ, data = dd_fb)),
  "Civic opportunity" = standardise(lm_robust(opc ~ race_per_white_nonhispanic, data = dd_fb)),
  "Rupasingha et al Index" = standardise(lm_robust(sk2014 ~ race_per_white_nonhispanic, data = dd_fb)),
  "Absolute difference between two measures" = standardise(lm_robust(diff ~ race_per_white_nonhispanic, data = dd_fb))
  )
```
...
modelsummary(mod_comp_list,
  ...
  output = here("tables", "table_s7.docx")
)
```
